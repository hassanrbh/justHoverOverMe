...
View.prototype.exercise3 = function() {
    ...
    let $love_jquery = $("<h1></h1>");
    let love_jquery = $love_jquery.text("I love jquery");

    $("#easel").append(love_jquery);
};

View.prototype.exercise4 = function() {
    //Challenge: Write your first name in every other square.
    //Result: Your name appears in every other square.
    $(".square").data("first_name", "hassan");

    let $span_first_name = $("<span></span>")
    let span_name = $span_first_name.text($('.square').data("first_name"));

    $(".square").append(span_name)
};
...
View.prototype.exercise5 = function() {
    ...
    $(".row").on("click", ".square", event => {
        let target = event.currentTarget;
        $regextCurrentTarget = $(target).data("pos").match(/\d/g);
        $colIdx = $regextCurrentTarget[0];
        $rowIdx = $regextCurrentTarget[1];
...
View.prototype.exercise6 = function() {
    ...
    $(".square").css({
        'background-color': window._randomColorString,
    })
};
